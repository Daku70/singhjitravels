...
const BookingForm = () => {
  ...
  const filterCities = (input: string) => {
    if (!input) return [];
    return indianCities.filter(city => city.toLowerCase().includes(input.toLowerCase())).slice(0, 5);
  };
  ...
  }} className="pt-24 pb-20 px-4 relative min-h-screen bg-cover bg-center bg-no-repeat my-[100px]">
      {/* Dark overlay for better text readability */}
      <div className="absolute inset-0 bg-black/50 mx-0 my-[100px]"></div>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  field
                        ...
                        <FormControl>
                          <Input placeholder="Origin location" {...field} onChange={e => {
                      field.onChange(e);
                      handleFromChange(e.target.value);
                    }} onFocus={() => handleFromChange(field.value)} onBlur={() => setTimeout(() => setShowFromSuggestions(false), 200)} />
                        </FormControl>
                        {showFromSuggestions && <div className="absolute top-full left-0 right-0 z-50 bg-popover border border-border rounded-md shadow-lg mt-1">
                            {fromSuggestions.map((city, index) => <div key={index} className="px-3 py-2 hover:bg-accent hover:text-accent-foreground cursor-pointer text-sm" onClick={() => {
                      field.onChange(city);
                  ...
                  field
                        ...
                        <FormControl>
                          <Input placeholder="Destination" {...field} onChange={e => {
                      field.onChange(e);
                      handleToChange(e.target.value);
                    }} onFocus={() => handleToChange(field.value)} onBlur={() => setTimeout(() => setShowToSuggestions(false), 200)} />
                        </FormControl>
                        {showToSuggestions && <div className="absolute top-full left-0 right-0 z-50 bg-popover border border-border rounded-md shadow-lg mt-1">
                            {toSuggestions.map((city, index) => <div key={index} className="px-3 py-2 hover:bg-accent hover:text-accent-foreground cursor-pointer text-sm" onClick={() => {
                      field.onChange(city);
